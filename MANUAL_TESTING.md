...
13. Verify `Version 1.2.4` appears at the top-right of the header and matches About Pasteboard and the built app's bundle metadata.
...
6. Focus the history list and verify Up/Down changes the selected row, Return restores and automatically pastes it, Space toggles its pin, and Delete removes it without requiring a pointer. Reopen history and verify Return in the search field does not restore the previous selection.
