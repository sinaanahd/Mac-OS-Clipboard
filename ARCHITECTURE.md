...
Region capture delegates selection to the built-in `/usr/sbin/screencapture -i` workflow. After that process exits successfully, a bounded asynchronous loader polls the private temporary output until a valid PNG has the same byte count on consecutive reads, then imports it into owned image storage, publishes the PNG to the general pasteboard for immediate reuse, and removes the temporary file.
